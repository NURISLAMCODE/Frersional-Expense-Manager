...
class DatabaseProvider with ChangeNotifier {
  ...
  Future<List<ExpenseCategory>> fecthCategories() async {
    final db = await database;
    return await db.transaction((txn) async {
      return await txn.query(cTable).then((data) {
        final Converter = List<Map<String, dynamic>>.from(data);

        List<ExpenseCategory> nlist = List.generate(Converter.length,
            (index) => ExpenseCategory.fromString(Converter[index]));
...
//method to add an expense to database
  ...
  Future<void> addExpense(Expense exp) async {
    ...
    await db.transaction((txn) async {
      await txn
        ...
        //var data = calculateEnteriesAndAmount(exp.category);
        updateCategory(
  ...
  Future<List<Expense>> fetchAllExpenses() async {
    ...
    return await db.transaction((txn) async {
      return await txn.query(eTable).then((data) {
        final Converter = List<Map<String, dynamic>>.from(data);
        List<Expense> nList = List.generate(
            Converter.length, (index) => Expense.fromString(Converter[index]));
        _expenses = nList;
        return _expenses;
      });
    });
  ...
  Map<String, dynamic> calculateEnteriesAndAmount(String category) {
    double total = 0.0;
